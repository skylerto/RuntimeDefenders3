...
public class TextToPDF
{
	...
	/*
	 * What is new : the method of detecting was for title,subtitle,linespacing
	 * but not 100 (see line 162) % . Apperantly its adding an empty
	 * List<string> to the dynamic_array. I tried to make method to remove emtpy
	 * array but did not work I fixed the method of detecting subtitle ( not
	 * detected b string regex ). In fact it was being mached along with title
	 * method removal of dependency of linecount ( to detect title, substring,
	 * linespace) making it only dependable on regex . Plus I made sure that it
	 * only detects TITLE=string, subTITLE=string only no numbers,
	 * subTITLE=string, only no numbers , spacing=float number no string I added
	 * extra if statement for currY to move to add new page when needed
	 */
	/*
	 * what needed to be fixed or add : removal of empty list from dynamic array
	 * when concatinating of list from dynamic array , if the size of dynamic
	 * array is even , it does all of them. If the size is odd it does not add
	 * the last one. added manually , check line 186 add feature to make
	 * variable line space
	 */
	...
	public static String INPUT_FILENAME = GUI4.fileToRead.getPath();
			...
			throws DocumentException, IOException
	{
		...
		cb.curveTo(tempx * 1.015f, tempy + (line_space * 0.4f),
				((x + line_space) + ((font_size / 1.8f) / 2.0f)) * (0.975f),
				tempy + (line_space * 0.4f), (x + (line_space * 1.0f))
