...
public class TabToPDFModel {
	/**
	 * Sample image creator - converts the first page of a pdf into png NOTE:
	 * Code only for testing basic jpedal functions
	 */
	/* CONSTANTS */
	...
	public static final String INPUT_FILENAME = "inputfiles/try.txt"; 
	public static final String PDF_FILENAME = "outputfiles/musicPDF.pdf"; 

	static final int MAX_DOC_INFO = 3; // There are 3 doc infos: Title,
										// Subtitle, Spacing
	...
	/**
	 * Runs the main method creatPDF() that reads the guitar tabs from the input
	 * file and converts it into a PDF file.
	 * 
	 * @param args
	 */

	// IMGCreatorv1_01 needs to make smaller preview image file, about a quarter
	// of the original size, maybe less
	public static void createPreview() {
		...
		try {
			...
			try {
				...
				// updates model's f preview image, need to change this to controller
				
				TabToPDFView.imageFile = outputfile;
				TabToPDFView.updateTopBox();
	...
	public void createPDF(String pdfname) throws DocumentException, IOException {
		...
		document = PdfWriter.getInstance(group1, new FileOutputStream(RESULT)); // PDF
																				// writer
		...
		/*
		 * Saves the data from the input file to: List<List<String>>
		 * dynamic_array
		 */
		readInput();

		/*
		 * Sets List<List<String>> outerconcat that will be used when writing to
		 * the file
		 */
		setOuter();
	...
	/**
	 * The method reads the input file and for the first few lines, extracts the
	 * document info and saves them to variables (title, subtitle, spacing).
		...
		try {
			...
			while ((line = reader.readLine()) != null) { // Read each line
				...
				if (linecount > MAX_DOC_INFO) {
					break;
					/*
					 * Detect a pattern and extract the title, subtitle and
					 * spacing
					 */
				} else {
					Matcher mtitle = (Pattern.compile(INPUT_TITLE_PATTERN))
							.matcher(line); // Title pattern to look for
					Matcher msub = (Pattern.compile(INPUT_SUBTITLE_PATTERN))
							.matcher(line); // Subtitle pattern to look for
					Matcher mspace = (Pattern.compile(INPUT_SPACING_PATTERN))
							.matcher(line); // Spacing pattern to look for
					if (mtitle.find()) // If the title pattern is found then set
										// the title
						this.title = mtitle.group(3);
					else if (msub.find()) // If the subtitle is found then set
											// the subtitle
						this.subtitle = msub.group(3);
					else if (mspace.find()) // If the spacing is found then set
											// the spacing
	...
	private void readInput() {
		...
		try {
			...
			while ((line = inputStream.readLine()) != null) { // Read each line
				if (linecount <= MAX_DOC_INFO) { // Skip the lines with document
													// info
					linecount++;
				} else {
					/*
					 * If line is not empty, delete all spaces and add line to a
					 * temp list
					 */
					if (!line.isEmpty()) {
					...
					/*
					 * If line is empty, then add the temp list to the larger
					 * list
					 */
	...
	/**
	 * Sets the outerconcat list which will be used when writing to the PDF
	 * file.
	 */
	...
	private void setOuter() {
		...
		for (int i = 0; i < dynamic_array.size() - 1; i += 2) {
			for (int j = 0; j < dynamic_array.get(i).size(); j++)
				inner.add(dynamic_array.get(i).get(j)
						+ dynamic_array.get(i + 1).get(j));
			outerconcat.add(new ArrayList<String>(inner));
			inner.clear();
		}
		if (dynamic_array.size() % 2 != 0)
			outerconcat.add(new ArrayList<String>(dynamic_array
					.get(dynamic_array.size() - 1)));
	}

	/**
	 * Iterates through the outerconcat list and adds the guitar tabs to the PDF
	 * file.
	 * 
	 * @throws DocumentException
	 * @throws IOException
	...
	private void writePDF() throws DocumentException, IOException {
		...
		for (List<String> bag : outerconcat) { // get the lists
			...
			for (int i = 0; i < bag.size(); i++) { // visit each string in the
													// that list

				DrawLine(0, currY, PDF_LEFTMARGIN_X, currY, cb); // draw border
																	// horizontal
																	// line
				for (int j = 0; j < bag.get(i).length(); j++) { // visit
																// character in
																// that string
																// for this list
																// to draw a
					...
					if (bag.get(i).charAt(j) == '|' && i < bag.size() - 1) { // i
																				// <
																				// bag.size()-1
																				// because
																				// for
						...
						DrawLine(currX, currY, currX, currY - PDF_LINE_SPACE,
								cb); // draw vertically line

					} else if (bag.get(i).charAt(j) == '|'
							&& i == bag.size() - 1) { // i == bag.size()-1
														// because we don't want
														// for last line to draw
														// vertically

						// DrawLine(currX,currY,currX,currY,cb); // don't draw

					} else if (bag.get(i).charAt(j) == '-'
							&& currX < PDF_RIGHT_MARGIN) { // draw horizontal
															// line for each -
															// and check for if
															// ends at 557

						DrawLine(currX, currY, currX + PDF_DASHLENGTH, currY,
								cb); // draw vertically with respect dashes to -
						currX = currX + PDF_DASHLENGTH; // update current
														// position of x to draw
														// the next dash or
														// character
					} else {

						InsertText(bag.get(i).charAt(j) + "", currX, currY - 2,
								cb);// write character taking account how many
									// points it takes
						currX = currX + PDF_DASHLENGTH;// update current
														// position of x to draw
														// the next dash or
					...
					if (currX >= PDF_RIGHT_MARGIN) { // draw the rest of
														// completing lines
	...
	private Paragraph addDocTitle() {
		...
		try {
			bf_title = BaseFont.createFont(BaseFont.HELVETICA, BaseFont.CP1250,
					BaseFont.EMBEDDED);
	 ...
	 * 
	 * @return The subtitle of the document. Returns null if no subtitle is
	 *         found.
	 */
	...
	private Paragraph addDocSubtitle() {
		...
		try {
			bf_subtitle = BaseFont.createFont(BaseFont.HELVETICA,
	...
	private static void InsertText(String text, float x, float y,
